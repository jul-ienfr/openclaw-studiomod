...
export function useWatcherController() {
  ...
  const loadSources = useCallback(async () => {
    try {
      ...
      if (data.sources) dispatch({ type: "HYDRATE_SOURCES", sources: data.sources });
    } catch (err) {
      dispatch({ type: "SET_ERROR", error: err instanceof Error ? err.message : "Failed to load sources" });
    }
  }, [dispatch]);
  ...
  const loadItems = useCallback(async (params?: Record<string, string | undefined>) => {
    try {
      ...
      dispatch({ type: "SET_ERROR", error: err instanceof Error ? err.message : "Failed to load items" });
    } finally {
      dispatch({ type: "SET_LOADING", loading: false });
    }
  }, [dispatch]);

  const loadScores = useCallback(async (params?: Record<string, string>) => {
    try {
      dispatch({ type: "SET_LOADING", loading: true });
      const qs = new URLSearchParams(params ?? {}).toString();
      const res = await fetch(`/api/watcher/scores?${qs}`);
      const data = await res.json();
      if (data.scores) dispatch({ type: "HYDRATE_SCORES", scores: data.scores, total: data.total });
    } catch (err) {
      dispatch({ type: "SET_ERROR", error: err instanceof Error ? err.message : "Failed to load scores" });
    } finally {
      dispatch({ type: "SET_LOADING", loading: false });
    }
  }, [dispatch]);

  const loadImplementations = useCallback(async (params?: Record<string, string>) => {
    try {
      const qs = new URLSearchParams(params ?? {}).toString();
      const res = await fetch(`/api/watcher/implementations?${qs}`);
      const data = await res.json();
      if (data.implementations) dispatch({ type: "HYDRATE_IMPLEMENTATIONS", implementations: data.implementations, total: data.total });
    } catch (err) {
      dispatch({ type: "SET_ERROR", error: err instanceof Error ? err.message : "Failed to load implementations" });
    }
  }, [dispatch]);
  ...
  return { loadSources, loadItems, loadScores, loadImplementations, loadNewItemsCount, triggerCheck, rollback, vacuum };
}
