...
async function get_handler(request: Request) {
  try {
    ...
    try {
      // Stats auteur
      const [total, autoCount, blockCount, implementedCount] = db.transaction(
        () => [
          (
            db
              .prepare(
                "SELECT COUNT(*) AS c FROM items WHERE LOWER(author) = ?",
              )
              .get(authorLower) as { c: number }
          ).c,
          (
            db
              .prepare(
      ...
      if ([...KNOWN_MAINTAINERS].some((m) => m.toLowerCase() === authorLower)) {
        ...
        [...KNOWN_BOTS].some((b) => b.toLowerCase() === authorLower) ||
