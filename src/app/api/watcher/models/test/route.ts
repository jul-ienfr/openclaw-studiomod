...
async function testModel(
  ...
  try {
    ...
    return { ok: false, latency_ms, error: `HTTP ${res.status}: ${errText.slice(0, 200)}` };
  } catch (err) {
    const latency_ms = Date.now() - start;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { models } = body as {
      models: Array<{ id: string; provider: string; model_id: string; base_url?: string; api_key?: string; timeout_seconds?: number }>;
    };
    ...
    const results = await Promise.all(
      models.map(async (m) => {
        const result = await testModel(m.provider, m.model_id, m.base_url, m.api_key, m.timeout_seconds ?? 30);
