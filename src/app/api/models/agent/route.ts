...
const AgentModelFullPatchSchema = z.object({
...
async function patch_handler(request: Request): Promise<NextResponse> {
  ...
  const parsed = await parseBody(request, AgentModelFullPatchSchema);
  if (isValidationError(parsed)) return parsed;

  const { agentId, primary, fallbacks } = parsed;
  ...
  if (primary === null && fallbacks.length === 0) {
    ...
    if (idx >= 0) {
      ...
      list.push({ id: agentId, model: modelValue });
    }
  }
