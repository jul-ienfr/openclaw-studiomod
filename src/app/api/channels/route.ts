...
import {
  ChannelsGetQuerySchema,
  ChannelsPatchSchema,
  ChannelsDeleteQuerySchema,
  ...
  try {
    ...
    try {
      ...
      for (const agentId of agentDirs.slice(0, 20)) {
        ...
        try {
          ...
          const entries = sessionsData.sessions as
    ...
    if (channelsRaw) {
      for (const [name, conf] of Object.entries(channelsRaw)) {
        ...
        let status: "connected" | "disconnected" | "error" | "configuring" =
          "disconnected";
...
async function patch_handler(request: Request) {
  const limited = applyRateLimit(request, RATE_LIMITS.channelsWrite);
