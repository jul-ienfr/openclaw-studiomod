...
function getKey(): Buffer {
  ...
  return crypto.scryptSync(process.env.HOSTNAME ?? "openclaw-studio", "openclaw-vault-salt", 32);
}
...
function decrypt(ciphertext: string): string {
  ...
  return decipher.update(Buffer.from(data, "base64")).toString("utf8") + decipher.final("utf8");
}
...
async function get_handler(request: Request) {
  ...
  try {
    ...
    const credentials = JSON.parse(plaintext) as Array<{ key: string; value: string }>;
    return NextResponse.json({ credentials });
  } catch (err) {
    return NextResponse.json({ error: String(err), credentials: [] }, { status: 500 });
  }
}
